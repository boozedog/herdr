...
## Agent panel space scope

When enabled, the Agents panel lists only agents that belong to the currently active worktree space. If the active workspace is not part of a worktree space, only that workspace's agents appear. Disabled by default; edit config and reload to toggle:
...
The existing grouped / priority sort chip still applies to the filtered list. There is no Settings UI or scope chip for this experiment.
