...
- Added opt-in `[experimental] agent_panel_space_scope` to filter the Agents panel to the active worktree space (or the active workspace when it has no space). Default: false.
